...
from Models import data_extraction, GPR_model, PLSR_model, DNN_model
import datetime
import warnings
...
def partial_obs_GPR_PLSR_ANN(tr, tr_obs, refl_obs,train_size, n_iterations, data_type):
    X = refl_obs
    y = tr_obs
    ...
    for iteration in range(n_iterations):
        ...
        ### ANN estimation
        pred_ann = DNN_model(X_train, y_train, X_test, y_test, tr, data_type,iteration)[0]
        pred_ann = pd.DataFrame(pred_ann,columns = ['predicted_ann'])
        
        y_test.reset_index(drop = True, inplace = True)
        pred = pd.concat([y_test,pred_gpr, pred_plsr, pred_ann],axis = 1)
    ...
    iterative_pred.to_csv(f'/scratch/fji7/transfer_learning_paper/3_results/{tr}/3_{tr}_{str(int(train_size*100))}% train GPR_PLSR_ANN_df_{data_type}.csv',index = False)
    vip_score.to_csv(f'/scratch/fji7/transfer_learning_paper/3_results/{tr}/3_{tr}_{str(int(train_size*100))}% train GPR_PLSR_ANN_VIP_{data_type}.csv',index = False)
    plsr_coef.to_csv(f'/scratch/fji7/transfer_learning_paper/3_results/{tr}/3_{tr}_{str(int(train_size*100))}% train GPR_PLSR_ANN_coefficients_{data_type}.csv',index = False)
    return
...
refl = pd.read_csv(f'/scratch/fji7/transfer_learning_paper/0_datasets/{tr}_reflectance_{data_type}.csv')
traits = pd.read_csv(f'/scratch/fji7/transfer_learning_paper/0_datasets/{tr}_traits_{data_type}.csv', dtype = {'Sample date': object})
...
for train_size in ts:
    p = pool.apply_async(func=partial_obs_GPR_PLSR_ANN,args= (tr, tr_obs, refl_obs,train_size, 10, data_type))
