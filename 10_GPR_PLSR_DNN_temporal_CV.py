...
def GPR_PLSR_ANN_temporal_CV(tr, tr_obs, refl_obs,n_splits,n_iterations):
    X = refl_obs
    y = tr_obs
    ...
    for train_index, test_index in kf.split(time):
        ...
        for iteration in range(n_iterations):
            print(tr, f'Fold{k+1} --- iteration {iteration+1}---train_season:{train_time},train_samples:{len(X_train)} --- test_seasons:{test_time}, test_samples:{len(X_test)}')
            ...
            ### ANN estimation
            trans_type = f'temporal transferability Fold{k+1}_iteration {iteration+1}'                                 
    ...
    df_within.to_csv(f'/scratch/fji7/transfer_learning_paper/3_results/{tr}/9_{tr}_temporal CV train GPR_PLSR_ANN_df_within.csv',index = False)   
    df_out.to_csv(f'/scratch/fji7/transfer_learning_paper/3_results/{tr}/9_{tr}_temporal CV train GPR_PLSR_ANN_df_out.csv',index = False)
    vip_score.to_csv(f'/scratch/fji7/transfer_learning_paper/3_results/{tr}/9_{tr}_temporal CV train GPR_PLSR_ANN_VIP.csv',index = False)
    plsr_coef.to_csv(f'/scratch/fji7/transfer_learning_paper/3_results/{tr}/9_{tr}_temporal CV train GPR_PLSR_ANN_coefficients.csv',index = False)
    return
...
tr_name = ["Chla+b", 'Ccar','LMA']
n_splits = 3

for tr in tr_name:
    refl = pd.read_csv(f'/scratch/fji7/transfer_learning_paper/0_datasets/{tr}_reflectance_temporal.csv')
    traits = pd.read_csv(f'/scratch/fji7/transfer_learning_paper/0_datasets/{tr}_traits_temporal.csv', dtype = {'Sample date': object})

    traits.loc[(traits['datasets']=='Dataset#7')&((traits['DOY']<165)|(traits['DOY']==165)),'season']='early growing season'
    traits.loc[(traits['datasets']=='Dataset#7')&((traits['DOY']>240)|(traits['DOY']==240)),'season']='post-peak season'
    traits.loc[(traits['datasets']=='Dataset#7')&(traits['DOY']>165)&(traits['DOY']<240),'season']='peak growing season'
    traits.loc[(traits['datasets']=='Dataset#8')&((traits['DOY']<151)|(traits['DOY']==151)),'season']='early growing season'
    traits.loc[(traits['datasets']=='Dataset#8')&((traits['DOY']>243)|(traits['DOY']==243)),'season']='post-peak season'
    traits.loc[(traits['datasets']=='Dataset#8')&(traits['DOY']>151)&(traits['DOY']<243),'season']='peak growing season'
    tr_obs, refl_obs = tr_obs, refl_obs = data_extraction(traits, refl, tr)
    
    p = pool.apply_async(func=GPR_PLSR_ANN_temporal_CV,args= (tr, tr_obs, refl_obs, n_splits, 10))
