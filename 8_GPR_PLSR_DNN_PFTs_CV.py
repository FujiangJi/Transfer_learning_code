...
def GPR_PLSR_ANN_PFTs_CV(tr, tr_obs, refl_obs,n_splits,n_iterations):
    X = refl_obs
    y = tr_obs
    ...
    for train_index, test_index in kf.split(pfts):                                  
        ...
        for iteration in range(n_iterations):
            ...
            ### ANN estimation
            trans_type = f'PFT transferability Fold{k+1}_iteration {iteration+1}'                             
    ...
    df_within.to_csv(f'/scratch/fji7/transfer_learning_paper/3_results/{tr}/7_{tr}_PFTs CV train GPR_PLSR_ANN_df_within.csv',index = False)   
    df_out.to_csv(f'/scratch/fji7/transfer_learning_paper/3_results/{tr}/7_{tr}_PFTs CV train GPR_PLSR_ANN_df_out.csv',index = False)
    vip_score.to_csv(f'/scratch/fji7/transfer_learning_paper/3_results/{tr}/7_{tr}_PFTs CV train GPR_PLSR_ANN_VIP.csv',index = False)
    plsr_coef.to_csv(f'/scratch/fji7/transfer_learning_paper/3_results/{tr}/7_{tr}_PFTs CV train GPR_PLSR_ANN_coefficients.csv',index = False)
    return
...
for tr in tr_name:
    refl = pd.read_csv(f'/scratch/fji7/transfer_learning_paper/0_datasets/{tr}_reflectance_PFT.csv')
    traits = pd.read_csv(f'/scratch/fji7/transfer_learning_paper/0_datasets/{tr}_traits_PFT.csv', dtype = {'Sample date': object})
    ...
    p = pool.apply_async(func=GPR_PLSR_ANN_PFTs_CV,args= (tr, tr_obs, refl_obs, n_splits, 10))
